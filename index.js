...
class CribDrag {
  constructor(salsa20file) {
    this.salsa20 = fs
      .readFileSync(salsa20file, "utf8", (err, data) => data)
      .split("\r\n");
  ...
  hexToBytes(hex) {
    return Array.from(hex).reduce(
      (acc, curr, i, arr) =>
        i % 2 ? acc : [...acc, parseInt(curr + arr[i + 1], 16)],
      []
    );
  }
  ...
  result() {
    const text = fs
      .readFileSync("result.csv", "utf8", (err, data) => data)
      .split("\r\n")
...
const result = new CribDrag("salsa20.txt")
  .guess("T", 2, 0)
  .guess("h", 2, 1)
  .guess("e", 2, 2)
  .guess(" ", 2, 3)
